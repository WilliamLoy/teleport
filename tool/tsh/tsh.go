...
type CLIConf struct {
	...
	// AWSRoleName is Amazon Role ANR that will be used for AWS CLI access.
	AWSRoleARN string
...
func Run(args []string, opts ...cliOption) error {
	...
	appLogin.Flag("aws-role-arn", "Amazon role ANR").StringVar(&cf.AWSRoleARN)
	appLogin.Flag("aws-role-name", "Amazon role name").StringVar(&cf.AWSRoleName)
